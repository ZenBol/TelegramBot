...
def start(message: types.Message) -> None:
    """
          Обрабатывает команду /train и запрашивает у пользователя город вылета.

          :param message: Сообщение от пользователя, содержащие команду /train.
          """
    log_user_request('/train', message.from_user.id)
    bot.send_message(message.chat.id, "Добро пожаловать! Введите город отправления:")
...
def get_travel_date(message: types.Message, from_city: str, to_city: str) -> None:
    ...
    transport_name = transport_dict["train"]
